...
const inputClass = 'mt-2 h-11 w-full rounded-md border border-input bg-card px-3.5 text-sm text-foreground outline-none transition-[border-color,box-shadow] placeholder:text-muted-foreground/65 focus:border-primary focus:ring-2 focus:ring-primary/10 disabled:cursor-not-allowed disabled:opacity-60';
...
function SectionHeading({ icon: Icon, eyebrow, title, description }: { icon: typeof FileCheck2; eyebrow: string; title: string; description: string }) {
  return <div className="flex items-start gap-3"><div className="grid size-9 shrink-0 place-items-center rounded-md bg-secondary text-primary"><Icon size={17} /></div><div><div className="font-mono text-[9px] font-bold uppercase tracking-[.18em] text-primary/80">{eyebrow}</div><h2 className="mt-1 text-base font-bold tracking-tight text-foreground">{title}</h2><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{description}</p></div></div>;
}
...
export default function NewCase({ role }: { role: Role }) {
  ...
  if (role !== 'Admin' && role !== 'Officer') {
    return <div data-testid="state-new-case-access-denied" className="mx-auto max-w-xl py-12 sm:py-20"><div className="rounded-lg border border-destructive/20 bg-card p-7 text-center shadow-sm sm:p-10"><div className="mx-auto grid size-14 place-items-center rounded-lg bg-destructive/10 text-destructive"><ShieldAlert size={27} /></div><div className="mt-5 font-mono text-[10px] font-bold uppercase tracking-[.2em] text-destructive">403 · restricted action</div><h1 className="mt-2 text-2xl font-bold tracking-tight text-foreground">Access Denied</h1><p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-muted-foreground">Your current role is not authorized to create a case. Contact an administrator if you need this permission.</p><Link href="/cases" data-testid="link-return-cases-access-denied" className="mt-7 inline-flex items-center gap-2 rounded-md bg-primary hover:bg-[#123A61] px-4 py-2.5 text-xs font-bold text-primary-foreground"><ArrowLeft size={14} />Return to Cases</Link></div></div>;
  }

  if (createdCase) {
    return <div data-testid="state-case-created" className="mx-auto max-w-xl py-12 sm:py-20"><div className="rounded-lg border border-[#16803C]/30 bg-card p-7 text-center shadow-sm sm:p-10"><div className="mx-auto grid size-14 place-items-center rounded-lg bg-[#16803C]/10 text-[#16803C]"><Check size={28} strokeWidth={2.5} /></div><div className="mt-5 font-mono text-[10px] font-bold uppercase tracking-[.2em] text-[#16803C]">Audit handoff complete</div><h1 className="mt-2 text-2xl font-bold tracking-tight text-foreground">Case Created Successfully</h1><p data-testid="text-case-created-success" className="mt-3 text-sm leading-relaxed text-muted-foreground">Case {createdCase.id} has been successfully created and assigned to {createdCase.officer}.</p><div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-primary"><span className="size-1.5 animate-pulse rounded-full bg-primary" />Opening protected case record</div></div></div>;
  }
  ...
  return <div data-testid="page-new-case" className="mx-auto max-w-[1160px] space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <h1 data-testid="heading-new-case" className="mt-3 text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Create New Case</h1>
        <p className="mt-1.5 text-sm text-muted-foreground">Create and assign a new legal or investigation case</p>
      </div>
      <Link href="/cases" data-testid="link-back-new-case" className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-3.5 py-2.5 text-xs font-bold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><ArrowLeft size={15} />Back to Cases</Link>
    </div>
    ...
    <Form {...form}>
      <form data-testid="form-new-case" onSubmit={(event) => { void form.handleSubmit(onSubmit)(event); }} className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
        <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
              ...
              <div className="mt-6 space-y-5">
                <div>
                  ...
                  <div className="relative">
                    ...
                    {availability !== 'unknown' && <span data-testid="status-case-id-availability" className={`absolute right-3 top-1/2 mt-1 flex -translate-y-1/2 items-center gap-1 text-[10px] font-bold ${availability === 'exists' ? 'text-destructive' : availability === 'checking' ? 'text-muted-foreground' : 'text-[#16803C]'}`}>{availability === 'checking' ? 'Checking' : availability === 'exists' ? 'In use' : <><Check size={12} />Available</>}</span>}
                  </div>
                ...
                <div>
                  ...
                  <textarea id="case-description" data-testid="textarea-new-case-description" {...register('description')} maxLength={DESCRIPTION_LIMIT} rows={5} className={`mt-2 min-h-[124px] w-full resize-y rounded-md border border-input bg-card px-3.5 py-3 text-sm leading-relaxed text-foreground outline-none transition-[border-color,box-shadow] placeholder:text-muted-foreground/65 focus:border-primary focus:ring-2 focus:ring-primary/10 ${errors.description ? errorInputClass : ''}`} placeholder="Summarize the matter and intended scope. Do not include document contents." />
            ...
            <section data-testid="section-case-controls">
              ...
              <div className="mt-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="mt-2 grid gap-2 sm:grid-cols-2">{CONFIDENTIALITY_LEVELS.map((option) => <button type="button" key={option} data-testid={`button-confidentiality-${option.toLowerCase().replaceAll('/', '-')}`} aria-pressed={confidentiality === option} onClick={() => setValue('confidentiality', option, { shouldValidate: true, shouldDirty: true })} className={`rounded-md border px-3 py-3 text-left transition-colors focus:outline-none focus:ring-2 focus:ring-primary/10 ${confidentiality === option ? 'border-primary bg-secondary text-primary' : 'border-border bg-card hover:border-primary/50'}`}><span className="flex items-center justify-between text-xs font-bold">{option}{confidentiality === option && <Check size={14} />}</span></button>)}</div>
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-border bg-muted/25 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7"><p className="text-[11px] text-muted-foreground"><RequiredMark /> Required fields</p><div className="flex gap-2 sm:ml-auto"><Link href="/cases" data-testid="link-cancel-new-case" className="inline-flex flex-1 items-center justify-center rounded-md border border-border bg-card px-4 py-2.5 text-xs font-bold hover:bg-muted sm:flex-none">Cancel</Link><button type="submit" data-testid="button-submit-new-case" disabled={saving || form.formState.isSubmitting} className="inline-flex flex-1 items-center justify-center gap-2 rounded-md bg-primary hover:bg-[#123A61] px-4 py-2.5 text-xs font-bold text-primary-foreground transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-55 sm:flex-none">{saving ? <><span className="size-3 animate-pulse rounded-full bg-current" />Creating Case...</> : <><Save size={14} />Create Case</>}</button></div></div>
        ...
        <aside className="space-y-5 lg:sticky lg:top-[100px]">
          <section data-testid="card-creation-summary" className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
            ...
            <div className="divide-y divide-border px-5 sm:px-6">{[['Case ID', watchedValues.caseId, 'summary-case-id'], ['Title', watchedValues.title || 'Not provided', 'summary-case-title'], ['Type', watchedValues.type || 'Not selected', 'summary-case-type'], ['Department', watchedValues.department || 'Not selected', 'summary-case-department'], ['Assigned officer', watchedValues.assignedOfficer || 'Not selected', 'summary-case-officer'], ['Priority', watchedValues.priority, 'summary-case-priority'], ['Start date', watchedValues.startDate || 'Not selected', 'summary-case-start-date'], ['Confidentiality', watchedValues.confidentiality, 'summary-case-confidentiality']].map(([label, value, testId]) => <div key={label} className="flex items-start justify-between gap-4 py-3"><span className="text-[11px] text-muted-foreground">{label}</span><span data-testid={testId} className="max-w-[62%] text-right text-xs font-bold text-foreground">{value}</span></div>)}</div>
            <div className="mx-5 mb-5 flex items-center justify-between rounded-md border border-[#16803C]/30 bg-[#16803C]/10 px-3 py-2.5 text-xs sm:mx-6"><span className="text-[#16803C] font-semibold">Initial status</span><span data-testid="summary-case-status" className="inline-flex items-center gap-1.5 font-bold text-[#16803C]"><span className="size-1.5 rounded-full bg-current" />Active</span></div>
          </section>
          <section className="rounded-lg border border-border bg-secondary/55 p-4"><div className="flex items-start gap-3"><div className="grid size-8 shrink-0 place-items-center rounded-md bg-card text-primary"><LockKeyhole size={15} /></div><div><h3 className="text-xs font-bold text-foreground">Protected by design</h3><p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">Only case metadata is collected here. Document contents remain inside the secure evidence workspace.</p></div></div></section>
        </aside>
      </form>
    </Form>
  </div>;
