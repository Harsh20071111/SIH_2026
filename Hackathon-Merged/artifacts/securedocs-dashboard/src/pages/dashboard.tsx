...
import { activities, caseStatuses, documentTypes, quickActions, stats, systemStatus } from '@/lib/mock-data';
...
function Donut() {
  const { getDashboardStats } = useSecurity();
  const { highRiskEvents, mediumRiskEvents, lowRiskEvents } = getDashboardStats();
  
  const riskDistribution = [
    { label: 'Low', value: Math.max(lowRiskEvents, 109), color: '#2f9b72' },
    { label: 'Medium', value: Math.max(mediumRiskEvents, 14), color: '#d49a28' },
    { label: 'High', value: Math.max(highRiskEvents, 5), color: '#c94b4b' },
  ];
  ...
  return <div className="relative mx-auto size-[146px] rounded-full" style={{ background: gradient }}><div className="absolute inset-[17px] grid place-items-center rounded-full bg-card"><div className="text-center"><div className="font-mono text-2xl font-bold">{total}</div><div className="text-[10px] text-muted-foreground">total cases</div></div></div></div>;
}

function AlertsPanel({ search }: Props) {
  const { events, updateEventStatus } = useSecurity();
  const alerts = events.filter(e => e.riskLevel === 'High' || e.riskLevel === 'Medium').map(e => ({
    id: e._id,
    severity: e.riskLevel === 'High' ? 'Critical' : 'High',
    title: e.type,
    description: e.details,
    meta: `${new Date(e.timestamp).toLocaleTimeString()} · ${e.userId} · ${e.caseId}`,
    score: e.riskScore,
    cta: 'Investigate',
    status: e.status
  }));
  
  const visible = alerts.filter((alert) => alert.status !== 'Resolved' && Object.values(alert).join(' ').toLowerCase().includes(search.toLowerCase()));
  return (
    <div className="rounded-xl border border-border bg-card shadow-sm">
      ...
      <div className="divide-y divide-border h-full max-h-[350px] overflow-y-auto">
        {visible.map((alert) => (
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-3">
                    <button data-testid={`button-alert-${alert.id}`} onClick={() => updateEventStatus(alert.id, 'Resolved')} className="text-[10px] font-bold text-muted-foreground hover:text-foreground">Mark resolved</button>
                    <Link href="/security" data-testid={`link-alert-cta-${alert.id}`} className="flex items-center gap-1 text-[10px] font-bold text-primary hover:underline">{alert.cta}<ChevronRight size={12} /></Link>
                  </div>
                </div>
              </div>
            </div>
...
export default function Dashboard({ search }: Props) {
  ...
  if (error) return <div data-testid="state-dashboard-error" className="flex min-h-[65vh] items-center justify-center"><div className="max-w-sm rounded-xl border border-red-200 bg-card p-8 text-center shadow-sm"><ShieldAlert className="mx-auto text-red-700" size={28} /><h1 className="mt-4 text-lg font-bold">Dashboard data unavailable</h1><p className="mt-2 text-sm text-muted-foreground">The local operations feed could not be reached. Nothing has been changed.</p><button data-testid="button-retry-dashboard" onClick={retry} className="mt-5 rounded-lg bg-primary px-4 py-2 text-xs font-bold text-primary-foreground hover:bg-primary/90">Retry connection</button></div></div>;
   return <div className="space-y-7">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.35fr_.9fr]"><section><SectionHeader eyebrow="Case posture" title="Risk distribution" action="View cases" href="/cases" /><div className="rounded-xl border border-border bg-card p-5 shadow-sm"><div className="flex items-center gap-6"><Donut /><div className="flex-1 space-y-4">
      {(() => {
        const { getDashboardStats } = useSecurity();
        const { highRiskEvents, mediumRiskEvents, lowRiskEvents } = getDashboardStats();
        ...
        return dynamicRiskDistribution.map((risk) => <div data-testid={`risk-${risk.label.toLowerCase()}`} key={risk.label} title={`${risk.label} risk: ${risk.value} cases`} className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="size-2.5 rounded-full" style={{ backgroundColor: risk.color }} /><span className="text-xs font-semibold">{risk.label} risk</span></div><span className="font-mono text-sm font-bold">{risk.value}</span></div>);
      })()}
      </div></div><div className="mt-4 rounded-lg bg-secondary/60 px-3 py-2 text-[10px] text-secondary-foreground"><span className="font-bold">Prioritise high-risk cases.</span> Cases need an active control review.</div><div className="mt-5 border-t border-border pt-4"><div className="mb-3 flex items-center justify-between"><span className="font-mono text-[9px] font-bold uppercase tracking-[.16em] text-muted-foreground">Case status</span><span className="text-[10px] text-muted-foreground">128 total</span></div><div className="flex h-2 overflow-hidden rounded-full bg-muted">{caseStatuses.map(([label, count, color]) => <div key={label} title={`${label}: ${count}`} className={`${color} h-full`} style={{ width: `${(count / 128) * 100}%` }} />)}</div><div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2">{caseStatuses.map(([label, count, color]) => <div data-testid={`case-status-${label.toLowerCase().replaceAll(' ', '-')}`} key={label} className="flex items-center justify-between text-[10px]"><span className="flex items-center gap-1.5 text-muted-foreground"><span className={`size-1.5 rounded-full ${color}`} />{label}</span><span className="font-mono font-bold">{count}</span></div>)}</div></div></div></section></div>
