...
export default function Dashboard({ search }: Props) {
  ...
  const greetingName = user?.name ? user.name.split(' ')[0] : 'Officer';
  const todayFormatted = new Intl.DateTimeFormat('en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(new Date());
