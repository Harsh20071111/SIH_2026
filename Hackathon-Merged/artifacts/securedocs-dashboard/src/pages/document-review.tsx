...
export default function DocumentReview({ id = 'C-1024' }: DocumentReviewProps) {
  ...
  useEffect(() => {
    documentService.getDocumentById(id).then(d => {
      setDoc(d);
      if (d.status === 'Approved') setActionStatus('approved');
      else if (d.status === 'Rejected') setActionStatus('rejected');
      else if (d.status === 'Flagged') setActionStatus('flagged');
      else setActionStatus('idle');
    }).catch(console.error);
  }, [id]);

  const docName = doc?.documentName || 'Evidence.pdf';
  const docCase = doc?.caseId || id;
  const docHash = doc?.hash || 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855';
  const docUploader = doc?.uploadedBy || 'Officer A';
  const docDate = doc?.uploadDate ? new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(doc.uploadDate)) : '01 Sept 2026';
          ...
          <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-[#E2E8F0] bg-[#FFFFFF] shadow-sm">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#E2E8F0] bg-[#F8FAFC] px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs font-bold text-[#111827]">{docName} Preview</span>
          ...
          <div className="rounded-2xl border border-[#E2E8F0] bg-[#FFFFFF] p-6 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3.5 text-xs">
              ...
              <div className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                ...
                <div className="mt-1 font-mono text-sm font-bold text-[#111827]">{docCase}</div>
              </div>

              {/* Document Name */}
              <div className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                <div className="text-[10px] font-semibold uppercase text-[#64748B]">Document Name</div>
                <div className="mt-1 font-mono text-sm font-bold text-[#2563EB] truncate" title={docName}>
                  {docName}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                ...
                <div className="mt-1 font-mono text-xs font-bold text-[#111827]">{docDate}</div>
              </div>
